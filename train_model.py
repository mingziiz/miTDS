...
from src.utils import Print, set_seeds, set_output, check_args, get_parameter_number, get_data
...
def main():
    ...
    dataset_train = get_dataset_from_configs(data_cfg, model_cfg, split_idx="train")
    dataset_val = get_dataset_from_configs(data_cfg, model_cfg, split_idx="val")
    ...
    trainer.set_optim_scheduler(run_cfg, params, model, iterator_train)
    ...
    for epoch in range(int(trainer.epoch), run_cfg.num_epochs):
        ...
        for B, batch in enumerate(iterator_train):
            trainer.train(batch, device, model_cfg)
        ...
        for B, batch in enumerate(iterator_val):
            trainer.evaluate(batch, device, model_cfg)
        ...
        trainer.log("val", output, dataset_train.set_labels, dataset_val.set_labels)
